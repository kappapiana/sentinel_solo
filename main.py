...
def build_timesheet_tab(page: ft.Page) -> ft.Control:
    ...
    def _on_timesheet_check(matter_id: int, checked: bool):
        ...
        if checked:
            ...
            timesheet_selected_ids |= get_descendant_matter_ids(matter_id)
        else:
    ...
    def _on_search_change(_):
        if timesheet_list_ref.current and timesheet_search_ref.current:
            timesheet_list_ref.current.controls = _build_timesheet_list_controls(timesheet_search_ref.current.value)
            page.update()
    ...
    def _do_export(_):
        if not timesheet_selected_ids:
            ...
            page.update()
            return
        only_not_invoiced = only_not_invoiced_ref.current.value if only_not_invoiced_ref.current else True
        entries = get_time_entries_for_export(timesheet_selected_ids, only_not_invoiced=only_not_invoiced)
        if not entries:
            page.snack_bar = ft.SnackBar(content=ft.Text("No matching time entries for the selected matters."))
            page.snack_bar.open = True
            page.update()
            return
        ...
        def _on_mark_yes(_):
            mark_entries_invoiced(entry_ids)
            dialog.open = False
        ...
        dialog = ft.AlertDialog(
            ...
            content=ft.Text(f"Timesheet exported to {out_path}. Do you want to mark the exported entries as invoiced?"),
            actions=[
...
def main(page: ft.Page) -> None:
    ...
    timesheet_tab = build_timesheet_tab(page)
    ...
    def show_timesheet(_):
        body_ref.current.content = timesheet_container
        page.update()
